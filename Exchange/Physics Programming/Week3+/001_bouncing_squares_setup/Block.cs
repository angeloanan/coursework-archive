...
public class Block : EasyDraw {
  ...
  public float Mass {
    get { return 4 * radius * radius * _density; }
  }
  ...
  public void SetFadeColor(float pRed, float pGreen, float pBlue) {
    _red = Mathf.Min(_red, pRed);
    _green = Mathf.Min(_green, pGreen);
    _blue = Mathf.Min(_blue, pBlue);
  }
    ...
    for (int i = 0; i < myGame.GetNumberOfMovers(); i++) {
      ...
      if (other != this) {
        ...
        if (Mathf.Abs(other.position.x - _position.x) < 25 &&
          ...
          SetFadeColor(1, 0.2f, 1);
          other.SetFadeColor(1, 0.2f, 1);
          if (wordy) {
